...
namespace {
	std::shared_ptr<std::set<int>> g_settings{ nullptr };
	std::mutex g_lock;
}

void LoadSettings() {
	std::shared_ptr<std::set<int>> newSettings{ std::make_shared<std::set<int>>() };

	{
		std::ifstream file{ L"j:\\temp\\foo.txt" };
		int itemCode;
		while (file >> itemCode) {
			newSettings->insert(itemCode);
		}
	}
...
BOOL __fastcall DROPFILTER_Main(D2UnitStrc* pItem)
{
	...
	if (g_settings->find(dwCode) != g_settings->end())
	{
		return TRUE;
	}
...
DWORD WINAPI Worker(LPVOID) {
	LoadSettings();
	FileWatcher fw{ L"j:\\temp", L"foo.txt", &LoadSettings };
	while (true) {
		fw.WatchFile();
