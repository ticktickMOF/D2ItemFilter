...
int __fastcall D2TEMPLATE_ApplyPatch(void* hGame, const SmallCodePatch* hPatch)
{
    while (hPatch->nDllFile != D2DLL_INVALID)
    {
        int nDll = hPatch->nDllFile;
...
int __stdcall DllAttach()
{
    ...
    D2TEMPLATE_ApplyPatch(hGame, gSmallCodePatches);
...
int __stdcall DllMain(void* hModule, unsigned long nReason, void* hReserved)
{
    switch (nReason)
    {
        case DLL_PROCESS_ATTACH:
            if (DllAttach() == 0)
                D2TEMPLATE_ErrorMessage("Couldn't attach to Diablo II");
        break;
    }
